...
export class IndexedDBAdapter implements StorageAdapter {
  ...
  private conversationSubjects = new Map<string, Subject<LocalConversation | null>>()
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(STORES.CONVERSATIONS)) {
          const convoStore = db.createObjectStore(STORES.CONVERSATIONS, { keyPath: 'id' })
        ...
        if (!db.objectStoreNames.contains(STORES.CERTIFICATES)) {
          ...
          certStore.createIndex('did_type_serial', ['did', 'type', 'serialNumber'], { unique: true })
        }

        // DIDs store
        if (!db.objectStoreNames.contains(STORES.DIDS)) {
          const didStore = db.createObjectStore(STORES.DIDS, { keyPath: 'did' })
  ...
  async updateMessage(id: string, updates: Partial<LocalMessage>): Promise<void> {
    ...
    const updated = {
      ...existing,
      ...updates,
      recipientIds: updates.recipientIds ? JSON.stringify(updates.recipientIds) : existing.recipientIds,
      reactions: updates.reactions ? JSON.stringify(updates.reactions) : existing.reactions,
  ...
  async updateConversation(
    ...
    const updated = {
      ...existing,
      ...updates,
      participantIds: updates.participantIds ? JSON.stringify(updates.participantIds) : existing.participantIds,
  ...
  observeConversation(convoId: string): Observable<LocalConversation | null> {
    if (!this.conversationSubjects.has(convoId)) {
      this.conversationSubjects.set(convoId, new Subject<LocalConversation | null>())
    }
  ...
  async listCertificates(did?: string): Promise<Array<{
    did: string
    type: ATSMSCertificateType
    serialNumber: string
    notBefore: Date
