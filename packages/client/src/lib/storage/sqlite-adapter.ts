...
import {
  type SQLiteDatabase,
  type StorageAdapter} from './interface'
import {
  type ConversationFilter,
  type LocalConversation,
  type LocalMessage} from './types'
...
export class SQLiteAdapter implements StorageAdapter {
  ...
  async getConversations(
    ...
    if (cursor) {
      query += ' WHERE lastMessageAt < (SELECT lastMessageAt FROM conversations WHERE id = ?)'
      params.push(cursor)
    }
  ...
  async deleteConversation(id: string): Promise<void> {
    this.db.transaction(() => {
      ...
      const deleteMessages = this.db.prepare('DELETE FROM messages WHERE convoId = ?')
      deleteMessages.run(id)

      // Delete the conversation
      const deleteConvo = this.db.prepare('DELETE FROM conversations WHERE id = ?')
      deleteConvo.run(id)
    })
  ...
  async findConversationByParticipants(participantDids: string[]): Promise<LocalConversation | null> {
    ...
    for (const row of rows) {
      ...
      if (sortedQuery.length === sortedParticipants.length &&
  ...
  private async getFilteredConversations(
    ...
    return allConvos.filter(convo => {
      ...
      if (filter.participantDid && !convo.participantIds.includes(filter.participantDid)) {
        return false
      }
      return true
    })
  ...
  async saveDid(
    ...
    const existingStmt = this.db.prepare('SELECT isPrimary, createdAt FROM dids WHERE did = ?')
