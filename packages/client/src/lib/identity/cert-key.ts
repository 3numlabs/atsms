 ...
 * record rkey pairing a device's cert with its prekey. Per identity-devices §2:
 * **the full 32-byte SHA-256 of the raw uncompressed public-key point**
 * (`0x04‖X‖Y`, the subjectPublicKey BIT STRING value) — not the whole-cert
 * hash, not the DER SPKI structure, and untruncated (so no RFC 7093 method
 * exactly). The x509 records, prekey records, worker inboxes, and JWT subject
 * all key on it (integration §8.5 re-keying, executed).
 */
