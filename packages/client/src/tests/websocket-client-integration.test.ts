  ...
  () => {
    ...
    test('should connect and authenticate with header-based auth (Node.js)', async () => {
      return new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(new Error('Test timeout: WebSocket did not connect within 10 seconds'))
        }, 10000)
        ...
        wsClient = new ATSMSWebSocketClient({
          ...
          getToken: async () => {
            ...
            return await generateJWT(endpointCert.certificatePrivateKeyPEM, certSerial, did)
          },
          onConnect: () => {
          ...
          onDisconnect: (code, reason) => {
            if (!connected) {
              clearTimeout(timeout)
              reject(new Error(`WebSocket disconnected before connecting: ${code} - ${reason}`))
            }
          }
        })
    ...
    test('should receive new_message broadcasts', async () => {
      ...
      return new Promise<void>(async (resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(new Error('Test timeout: Did not receive message broadcast within 15 seconds'))
        }, 15000)
        ...
        try {
          ...
          const testContent = Buffer.from('Test WebSocket broadcast message').toString('base64')
          ...
          if (!response.ok) {
            ...
            clearTimeout(timeout)
            reject(new Error(`Failed to send test message: ${response.status} - ${errorText}`))
            return
          }

          const result = await response.json()
          console.log(`Test message sent successfully, delivered to ${result.delivered} inbox(es)`)
        } catch (error) {
          clearTimeout(timeout)
          reject(error)
        }
    ...
    test('should handle disconnect and cleanup', async () => {
      ...
      return new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(new Error('Test timeout: Disconnect did not complete within 5 seconds'))
        }, 5000)
    ...
    test('should reconnect after disconnect (if max attempts not reached)', async () => {
      ...
      return new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(() => {
          ...
          reject(new Error('Test timeout: Reconnection did not complete within 20 seconds'))
        }, 20000)
        ...
        wsClient = new ATSMSWebSocketClient({
          ...
          onDisconnect: (code, reason) => {
            ...
            if (connectCount === 1 && !firstDisconnect) {
              ...
              console.log('First disconnect detected, waiting for reconnection...')
