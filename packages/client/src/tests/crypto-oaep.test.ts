...
describe('RSA-OAEP Encryption/Decryption', () => {
  test('should encrypt and decrypt message successfully', async () => {
    ...
    const rootCert = await ATSMSRootCertificate.generate('did:test:123', 'test.acme.xyz')
    const endpointCert = await rootCert.generateSignedEndpointCertificate('test@test.acme.xyz')
    
    // Test message
    const testMessage = 'Hello, World! This is a test message for RSA-OAEP encryption.'
    const messageBytes = new TextEncoder().encode(testMessage)
    
    // Encrypt the message
    const encryptedContent = await encryptMessageOAEP(messageBytes, [endpointCert])
    ...
    // Decrypt the message
    const decryptedContent = await decryptMessageOAEP(encryptedContent, endpointCert)
  ...
  test('should handle multiple recipients', async () => {
    // Generate test certificates for multiple recipients
    const rootCert = await ATSMSRootCertificate.generate('did:test:root', 'test.acme.xyz')
    const recipient1 = await rootCert.generateSignedEndpointCertificate('recipient1@recipient1.acme.xyz')
    const recipient2 = await rootCert.generateSignedEndpointCertificate('recipient2@recipient2.acme.xyz')
    ...
    const encryptedContent = await encryptMessageOAEP(messageBytes, [recipient1, recipient2])
  ...
  test('should fail to decrypt without private key', async () => {
    // Generate test certificates
    const rootCert = await ATSMSRootCertificate.generate('did:test:123', 'test.acme.xyz')
    const clientCertWithKey = await rootCert.generateSignedEndpointCertificate('test@test.acme.xyz')
    ...
    // Encrypt the message
    const encryptedContent = await encryptMessageOAEP(messageBytes, [clientCertWithKey])
  ...
  test('should fail to decrypt with wrong certificate', async () => {
    // Generate test certificates
    const rootCert = await ATSMSRootCertificate.generate('did:test:123', 'test.acme.xyz')
    const rightCert = await rootCert.generateSignedEndpointCertificate('right@right.acme.xyz')
    const wrongCert = await rootCert.generateSignedEndpointCertificate('wrong@wrong.acme.xyz')
    ...
    const encryptedContent = await encryptMessageOAEP(messageBytes, [rightCert])
  ...
  test('should handle binary data', async () => {
    ...
    // Encrypt and decrypt
    const encryptedContent = await encryptMessageOAEP(binaryData, [endpointCert])
    const decryptedContent = await decryptMessageOAEP(encryptedContent, endpointCert)
  ...
  test('should handle empty message', async () => {
    ...
    // Encrypt and decrypt
    const encryptedContent = await encryptMessageOAEP(emptyMessage, [endpointCert])
    const decryptedContent = await decryptMessageOAEP(encryptedContent, endpointCert)
  ...
  describe('encryptMessageOAEP specific tests', () => {
    ...
    test('should work with certificate without private key for encryption', async () => {
      ...
      const encryptedContent = await encryptMessageOAEP(testMessage, [clientCertNoKey])
      
      // Verify encryption worked
      expect(encryptedContent.length).toBeGreaterThan(0)
      
      // Should be able to decrypt with the certificate that has the private key
      const decryptedContent = await decryptMessageOAEP(encryptedContent, clientCertWithKey)
    ...
    test('should handle mixed recipient types', async () => {
      ...
      const rootCert = await ATSMSRootCertificate.generate('did:test:root', 'test.acme.xyz')
      const cert1WithKey = await rootCert.generateSignedEndpointCertificate('one@one.acme.xyz')
      const cert2WithKey = await rootCert.generateSignedEndpointCertificate('two@two.acme.xyz')
      
      // Create one without private key
      const cert2NoKey = ATSMSEndpointCertificate.fromPEM(cert2WithKey.certificatePEM)
      
      const testMessage = new TextEncoder().encode('Mixed recipients')
      
      // Encrypt for both (one with key, one without)
      const encryptedContent = await encryptMessageOAEP(testMessage, [cert1WithKey, cert2NoKey])
      
      // Both original certificates with keys should be able to decrypt
      const decrypted1 = await decryptMessageOAEP(encryptedContent, cert1WithKey)
      const decrypted2 = await decryptMessageOAEP(encryptedContent, cert2WithKey)
