...
describe('IndexedDBAdapter', () => {
  ...
  describe('Conversation Operations', () => {
    ...
    test('should save and retrieve a conversation', async () => {
      ...
      expect(retrieved?.participantIds).toEqual(['did:plc:alice', 'did:plc:bob'])
    })

    test('should find conversation by participants', async () => {
      await adapter.saveConversation(testConvo)

      const found = await adapter.findConversationByParticipants(['did:plc:bob', 'did:plc:alice'])
  ...
  describe('Certificate Operations', () => {
    test('should save and retrieve a certificate', async () => {
      ...
      const rootCert = await ATSMSRootCertificate.generate('did:plc:test', 'example.com')
      ...
      const retrieved = await adapter.getCertificate('did:plc:test', 'root', rootCert.serialNumber)
      expect(retrieved).toBeDefined()
    ...
    test('should delete a certificate', async () => {
      ...
      await adapter.deleteCertificate('did:plc:test', 'root', rootCert.serialNumber)
  ...
  describe('DID Management', () => {
    test('should save and retrieve primary DID', async () => {
      const rootCert = await ATSMSRootCertificate.generate('did:plc:alice', 'example.com')
    ...
    test('should preserve isPrimary when updating DID', async () => {
      ...
      await adapter.saveDid('did:plc:alice', 'alice.example.com', endpointCert1)
      ...
      await adapter.saveDid('did:plc:alice', 'alice.example.com', endpointCert2)
