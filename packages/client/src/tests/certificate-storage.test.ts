...
-----END PRIVATE KEY-----`
    ...
    const retrieved = await adapter.getCertificate(did, 'root', 'test-serial-123')
...
-----END PRIVATE KEY-----`
    ...
    const retrieved = await adapter.getCertificate(did, 'endpoint', 'test-client-456')
    expect(retrieved).toBeDefined()
    expect(retrieved?.certificatePEM).toBe(certificatePEM)
    ...
    // Decrypt and verify
    const decryptedKey = PasswordEncryption.decrypt(retrieved!.privateKeyPEM!, password)
...
-----END CERTIFICATE-----`
    ...
    await adapter.saveCertificate(did, 'endpoint', 'client-001', certificatePEM, 'private-key', false)
    await adapter.saveCertificate(did, 'endpoint', 'client-002', certificatePEM)
...
-----END CERTIFICATE-----`
  ...
  test('PasswordEncryption should validate correct password', () => {
    ...
    expect(PasswordEncryption.validate(encrypted, 'wrong-password')).toBe(false)
  })
