...
describe("Crypto Functions", () => {
  ...
  describe("Message Signing and Encryption", () => {
    ...
    test("should sign a message", async () => {
      ...
      const signedContent = await signMessage(message, signerCert);
    ...
    test("should decrypt and verify a message", async () => {
      ...
      const signedContent = await signMessage(originalMessage, signerCert);
    ...
    test("should handle JSON message payload", async () => {
      ...
      const signedContent = await signMessage(messageString, signerCert);
    ...
    test("should encrypt for multiple recipients", async () => {
      ...
      const endpointCert = ATSMSEndpointCertificate.fromPEM(clientCertPEM);
      const recipient2Cert = ATSMSEndpointCertificate.fromPEM(
        recipient2Result.cert,
      );
      ...
      const recipient2CertWithKey =
        await ATSMSEndpointCertificate.fromPEMWithKey(
          recipient2Result.cert,
          recipient2Result.privateKey,
        );
  ...
  describe("Error Handling", () => {
    ...
    test("should throw error when signing with mismatched key and cert", async () => {
      const clientCertPEM = readFileSync(
        path.join(testDir, "client-cert.pem"),
        "utf8",
      );
...
-----END PRIVATE KEY-----`;
      ...
      expect(
        (async () => {
          const wrongCert = await ATSMSEndpointCertificate.fromPEMWithKey(
            clientCertPEM,
            wrongKeyPEM,
          );
    ...
    test("should throw error when decrypting with wrong key", async () => {
      ...
      const clientKeyPEM = readFileSync(
        path.join(testDir, "client-key.pem"),
        "utf8",
      );
      ...
      const wrongRecipientCert = await ATSMSEndpointCertificate.fromPEMWithKey(
        wrongRecipientResult.cert,
        wrongRecipientResult.privateKey,
      );

      expect(
        decryptAndVerifyMessageSignature(encryptedContent, wrongRecipientCert),
