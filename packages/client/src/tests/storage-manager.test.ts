...
-----END PRIVATE KEY-----`,
      ...
      await managerWithAPI.saveDid('did:plc:user123', 'user.test', endpointCert)
...
-----END PRIVATE KEY-----`,
  ...
  describe('Event Emitters', () => {
    ...
    test('should emit conversation updated event', async () => {
      ...
      ;(managerWithEvents as any).conversationUpdatedSubject.next('updated-convo')
  ...
  describe('Certificate Caching', () => {
    test('should cache certificates after fetching', async () => {
      ...
      const cache1 = await (managerWithAPI as any).getCachedOrFetchCertificatesForDID('did:plc:test')
      expect(cache1.did).toBe('did:plc:test')

      // Second call should use cache
      const cache2 = await (managerWithAPI as any).getCachedOrFetchCertificatesForDID('did:plc:test')
...
-----END PRIVATE KEY-----`,
      ...
      await freshManager.saveDid('did:plc:alice123', 'alice.test', mockEndpointCert)
...
-----END PRIVATE KEY-----`,
      ...
      await freshManager.saveDid('did:plc:alice123', 'alice.test', mockEndpointCert1)
      ...
      await freshManager.saveDid('did:plc:alice123', 'alice.test', mockEndpointCert2)
...
-----END PRIVATE KEY-----`,
    ...
    test('should throw error starting transport for non-existent DID', async () => {
      ...
      ).rejects.toThrow('Cannot start transport: DID did:plc:nonexistent not found')
    })
