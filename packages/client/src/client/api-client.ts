...
async function loadOrGenerateJWT(handle: string): Promise<string> {
  ...
  try {
    ...
    console.log(`${colors.yellow}Note: Using JWT from old location (${oldTokenPath})${colors.reset}`)
    console.log(`${colors.yellow}Consider regenerating JWT to use new cache location${colors.reset}`)
  ...
  try {
    ...
    const authCache: AuthCache = JSON.parse(readFileSync(authCachePath, 'utf8'))
    ...
    if (cert.privateKeyEncrypted === 1) {
      throw new Error('Certificate private key is encrypted - cannot auto-generate JWT')
    }
    ...
    console.log(`${colors.green}✓ Generated new JWT from cached credentials${colors.reset}`)
    return token

  } catch (error: any) {
    // Could not generate JWT
    console.error(`${colors.red}Error: Could not load or generate JWT token${colors.reset}`)
    ...
    console.error(`  Or: ${colors.cyan}bun src/client/atsms-chat.ts ${handle}${colors.reset}`)
    console.error('')
    console.error(`${colors.yellow}This will authenticate and cache credentials in ~/.atsms${colors.reset}`)
    process.exit(1)
  }
}
    ...
    const req = httpsRequest(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
          ...
          console.error(`${colors.red}Error: HTTP ${res.statusCode}${colors.reset}`)
          try {
...
// API request helper
async function apiRequest(method: string, path: string, data: any = null, token: string): Promise<any> {
...
type CommandHandler = (token: string, jwtInfo: JWTInfo, ...args: string[]) => Promise<void>
...
const commands: Record<string, CommandHandler | AuthCommandHandler> = {
  async auth(handle: string) {
    ...
    console.log(`${colors.yellow}Authenticating and generating new JWT for ${handle}...${colors.reset}`)
    console.log('')
    console.log(`${colors.red}Note: The auth command requires the AT-SMS client to be installed.${colors.reset}`)
    console.log(`${colors.yellow}Please use the main AT-SMS client instead:${colors.reset}`)
    console.log('')
    console.log(`  ${colors.cyan}bun run atsms login ${handle}${colors.reset}`)
    console.log('')
    console.log('This will authenticate with AT Protocol and generate a new JWT token.')
    console.log('')
    console.log(`${colors.yellow}Alternative: If JWT is expired but you have valid certificates:${colors.reset}`)
    console.log(`  1. Run: ${colors.cyan}bun run atsms login ${handle}${colors.reset}`)
  ...
  async list(token: string, jwtInfo: JWTInfo, limitStr = '50', afterStr?: string) {
    console.log(`${colors.yellow}Listing messages...${colors.reset}`)
    const limit = parseInt(limitStr, 10)
    let query = `?limit=${limit}`
    if (afterStr) query += `&after=${afterStr}`

    const result: ListMessagesResponse = await apiRequest('GET', `/messages/${jwtInfo.did}/${jwtInfo.certSerial}${query}`, null, token)

    console.log(`${colors.green}Messages (${result.totalCount} total):${colors.reset}`)
    result.messages.forEach(msg => {
      const timestamp = new Date(msg.storedAt).toLocaleString()
      console.log(`  ${colors.cyan}[${msg.seq}]${colors.reset} ${msg.id} - ${msg.seq} - (${timestamp})`)
    })

    if (result.hasMore) {
      console.log(`${colors.yellow}More messages available. Use --after ${result.latestSeq} to see more.${colors.reset}`)
    }
  },
  ...
  async get(token: string, jwtInfo: JWTInfo, messageId?: string) {
    ...
    console.log(`${colors.yellow}Getting message ${messageId}...${colors.reset}`)
    const result = await apiRequest('GET', `/messages/${jwtInfo.did}/${jwtInfo.certSerial}/${messageId}`, null, token)
    console.log(JSON.stringify(result, null, 2))
  },
  ...
  async delete(token: string, jwtInfo: JWTInfo, messageId?: string) {
    ...
    console.log(`${colors.yellow}Deleting message ${messageId}...${colors.reset}`)
    await apiRequest('DELETE', `/messages/${jwtInfo.did}/${jwtInfo.certSerial}/${messageId}`, null, token)
  ...
  async stats(token: string, jwtInfo: JWTInfo) {
    ...
    const result: StatsResponse = await apiRequest('GET', `/messages/${jwtInfo.did}/${jwtInfo.certSerial}/stats`, null, token)

    console.log(`${colors.green}Inbox Statistics:${colors.reset}`)
    console.log(`  Total messages: ${colors.cyan}${result.messageCount}${colors.reset}`)
    console.log(`  Unread messages: ${colors.cyan}${result.unreadCount}${colors.reset}`)
    console.log(`  Latest sequence: ${colors.cyan}${result.latestSeq}${colors.reset}`)
    console.log(`  Connected clients: ${colors.cyan}${result.connectedClients}${colors.reset}`)
  },

  async send(token: string, jwtInfo: JWTInfo, encryptedContentPath?: string, ...recipientArgs: string[]) {
    if (!encryptedContentPath || recipientArgs.length === 0) {
      console.error(`${colors.red}Error: Encrypted content file and at least one recipient required${colors.reset}`)
      console.error(`${colors.yellow}Usage: send <handle> <encrypted-content-file> <recipient1> [recipient2] ...${colors.reset}`)
      console.error(`${colors.yellow}Recipient format: did:certSerial:email${colors.reset}`)
      console.error(`${colors.yellow}Example: send aib0b.bsky.social ./message.p7m did:plc:abc:4d18ac7f:user@atsms.example.com${colors.reset}`)
      process.exit(1)
    }
    ...
    try {
      ...
      console.error(`${colors.red}Error: Could not read encrypted content file: ${error.message}${colors.reset}`)
      process.exit(1)
    }
    ...
    for (const arg of recipientArgs) {
      ...
      if (parts.length < 4) {
        console.error(`${colors.red}Error: Invalid recipient format: ${arg}${colors.reset}`)
        console.error(`${colors.yellow}Expected: did:plc:xxx:certSerial:email${colors.reset}`)
        process.exit(1)
      }
    ...
    const recipients: Recipient[] = Array.from(recipientMap.entries()).map(([did, endpoints]) => ({
      did,
      endpoints
    }))
    ...
    console.log(`${colors.yellow}Connecting to WebSocket to send message...${colors.reset}`)
    console.log(`${colors.blue}Recipients: ${uniqueDids} DID(s) with ${totalEndpoints} endpoint(s)${colors.reset}`)
    ...
    ws.on('message', (data) => {
      try {
        ...
        switch (msg.type) {
          ...
          case 'send_response':
            if (msg.requestId === requestId) {
              ...
              msg.results?.forEach(result => {
                const statusColor = result.status === 'sent' ? colors.green : colors.red
                console.log(`  ${result.email} (${result.did}:${result.certSerial})`)
                console.log(`    Status: ${statusColor}${result.status}${colors.reset}`)
                if (result.error) console.log(`    Error: ${colors.red}${result.error}${colors.reset}`)
              })
          ...
          case 'error':
            if (msg.requestId === requestId) {
              console.error(`${colors.red}Error sending message: ${msg.error}${colors.reset}`)
              ws.close()
              process.exit(1)
            }
            break
    ...
    ws.on('close', (code, reason) => {
      if (code === 1008) {
        console.log(`${colors.red}WebSocket closed: Authentication failed - ${reason}${colors.reset}`)
      } else {
  ...
  async watch(token: string, jwtInfo: JWTInfo) {
    console.log(`${colors.yellow}Connecting to WebSocket for real-time updates...${colors.reset}`)
    ...
    ws.on('open', () => {
      console.log(`${colors.green}✓ Connected to WebSocket${colors.reset}`)
      console.log(`${colors.yellow}Waiting for auth_required message...${colors.reset}`)
    ...
    ws.on('message', (data) => {
      try {
        ...
        switch (msg.type) {
          case 'auth_required':
            console.log(`${colors.yellow}[${timestamp}] Authentication required (deadline: ${new Date(msg.deadline!).toLocaleTimeString()})${colors.reset}`)
            // Send auth token in response to auth_required
            console.log(`${colors.blue}Sending auth token (length: ${token.length})${colors.reset}`)
            ws.send(JSON.stringify({
              type: 'auth',
              token: token
            }))
            break
          case 'auth_success':
            console.log(`${colors.green}[${timestamp}] ✓ Authenticated successfully${colors.reset}`)
            console.log(`${colors.green}[${timestamp}] DID: ${msg.did}, Certificate: ${msg.certSerial}${colors.reset}`)
            console.log(`${colors.yellow}Listening for messages... (Press Ctrl+C to stop)${colors.reset}`)
            break
          case 'connected':
            console.log(`${colors.green}[${timestamp}] Connected successfully${colors.reset}`)
            break
          case 'new_message':
            console.log(`${colors.cyan}[${timestamp}] New message:${colors.reset} ${msg.message!.id} with seq ${msg.message!.seq}`)
            break
          case 'message_read':
            console.log(`${colors.blue}[${timestamp}] Message read:${colors.reset} ${msg.messageId}`)
            break
          case 'message_deleted':
            console.log(`${colors.magenta}[${timestamp}] Message deleted:${colors.reset} ${msg.messageId}`)
            break
          case 'pong':
            // Ignore pong messages
            break
          default:
            console.log(`${colors.yellow}[${timestamp}] ${msg.type}:${colors.reset}`, msg)
        }
      } catch {
        console.log(`${colors.yellow}[${new Date().toLocaleTimeString()}] Raw message:${colors.reset}`, data.toString())
      }
    })
...
function showUsage() {
  ...
  console.log('')
  console.log('Usage: bun src/client/api-client.ts <command> <handle> [args...]')
  console.log('')
  console.log('Commands:')
  console.log('  auth <handle>             - Authenticate and generate new JWT token')
  ...
  console.log('  send <handle> <encrypted-file> <recipient1> [recipient2] ... - Send message via WebSocket')
  console.log('  watch <handle>            - Connect to WebSocket for real-time updates')
  console.log('')
  ...
  console.log('  bun src/client/api-client.ts send aib0b.bsky.social ./message.p7m did:plc:abc:4d18ac7f:user@atsms.example.com')
...
async function main() {
  try {
    ...
    if (command === 'auth') {
      if (!handle) {
        console.error(`${colors.red}Error: Handle required for auth command${colors.reset}`)
        showUsage()
        process.exit(1)
      }
    ...
    console.log(`Certificate: ${colors.green}${jwtInfo.certSerial}${colors.reset}`)

    // Check token expiry
    const expiry = new Date(jwtInfo.exp * 1000)
    const now = new Date()
    if (expiry < now) {
      console.error(`${colors.red}Error: JWT token expired on ${expiry.toLocaleString()}${colors.reset}`)
      console.error(`${colors.yellow}Please run 'bun src/client/api-client.ts auth ${handle}' to refresh${colors.reset}`)
      process.exit(1)
    }
    console.log(`Token expires: ${colors.yellow}${expiry.toLocaleString()}${colors.reset}`)
    console.log('')
