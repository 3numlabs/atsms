...
import {createMessagePayload, prepareMessageForSending} from '../lib/messages.js'
import {
  type ATSMSConfig,
...
class ATSMSCLITool {
   ...
   */
  private async getJWTFromFiles(certPath: string, keyPath: string): Promise<string> {
    const certPEM = readFileSync(certPath, 'utf8')
    const keyPEM = readFileSync(keyPath, 'utf8')
    const endpointCert = await ATSMSEndpointCertificate.fromPEMWithKey(certPEM, keyPEM)
  ...
  async sendMessage(
    ...
    const senderCert = await ATSMSEndpointCertificate.fromPEMWithKey(senderCertPEM, senderKeyPEM)
    ...
    if (!recipientHandleOrDid.startsWith('did:')) {
      ...
      const response = await this.agent!.resolveHandle({handle: recipientHandleOrDid})
    ...
    const recipientCerts = await this.atsmsClient!.getUserCertificates(recipientDid)

    if (!recipientCerts.endpointCerts || recipientCerts.endpointCerts.length === 0) {
      throw new Error(`No endpoint certificates found for recipient ${recipientHandleOrDid}`)
    }
    console.log(`✓ Found ${recipientCerts.endpointCerts.length} recipient certificate(s)`)
  ...
  async receiveMessages(
    ...
    const endpointCert = await ATSMSEndpointCertificate.fromPEMWithKey(endpointCertPEM, endpointKeyPEM)
    ...
    for (const [index, msgMeta] of messageList.entries()) {
      ...
      try {
        ...
        // Decrypt
        const encryptedBytes = Uint8Array.from(atob(message.encryptedContent), c =>
          c.charCodeAt(0)
        )

        const decrypted: ATSMSDecryptedMessage = await decryptAndVerifyMessageSignature(
          encryptedBytes,
          endpointCert
        )

        const decryptedContent = new TextDecoder().decode(decrypted.decryptedContent)
  ...
  async createP7MFile(
    ...
    // Load recipient certificate (public only)
    const recipientCertPEM = readFileSync(recipientCertPath, 'utf8')
    const recipientCert = ATSMSEndpointCertificate.fromPEM(recipientCertPEM)
    ...
    const encryptedContent = await encryptMessage(signedContent, [recipientCert])
  ...
  async deleteMessage(
    ...
    await this.atsmsApiClient.deleteMessage(this.did, endpointCert.serialNumber, messageId)
  ...
  async getStats(
    ...
    const stats = await this.atsmsApiClient.getStats(this.did, endpointCert.serialNumber)
...
`)
  ...
  const config: ATSMSConfig = {
    apiUrl: (parsedArgs['api-url'] as string) || 'https://atsms-api.enumdao.workers.dev'
  }
  ...
  try {
    switch (command) {
      ...
      case 'receive': {
        ...
        await tool.receiveMessages(handle, password, endpointCert, endpointKey, outputDir)
        break
      }
      ...
      case 'list': {
        ...
        await tool.listMessages(handle, password, endpointCert, endpointKey, afterSeq)
        break
      }
      ...
      case 'download': {
        ...
        if (!handle || !endpointCert || !endpointKey || !messageId || !outputDir) {
          throw new Error(
      ...
      case 'create-p7m': {
        ...
        if (!senderCert || !senderKey || !recipientCert || !message || !output) {
          throw new Error(
            'Missing required arguments: --sender-cert, --sender-key, --recipient-cert, --message, --output'
          )
        }

        await tool.createP7MFile(senderCert, senderKey, recipientCert, message, output)
        break
      }
      ...
      case 'delete': {
        ...
        await tool.deleteMessage(handle, password, endpointCert, endpointKey, messageId)
        break
      }
