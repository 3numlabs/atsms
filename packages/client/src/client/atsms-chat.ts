    ...
    if (this.replActive) {
      this.rl.on('line', async (input) => {
        ...
        if (trimmed.startsWith('/')) {
          ...
          console.log(colors.warning('⚠ You need to be in a conversation to send messages'))
          console.log(colors.info('Use /msg <handle> to start a conversation, or type /help for all commands'))
        }
...
`))
  ...
  private async promptForHandle(): Promise<string> {
    return new Promise((resolve) => {
      this.rl.question(colors.prompt('Enter your handle or DID (e.g., alice.bsky.social): '), (answer) => {
        resolve(answer.trim())
      })
    })
  }
  ...
  private async authenticate(identifier: string) {
    try {
      ...
      if (cached && cached.expiresAt > Date.now()) {
        ...
        try {
          if (this.debug) {
            console.log(colors.success('[DEBUG] About to call setupSession...'))
          }
          await this.setupSession(cachedHandle, cached.did, cached.accessJwt, cached.refreshJwt, pdsUrl)
          return
        } catch (error: any) {
          console.log(colors.error(`[CAUGHT ERROR] ${error?.error || error?.message || error}`))
          // Check if token expired - try to refresh
          const isExpiredToken = error?.error === 'ExpiredToken' ||
                                 error?.message?.includes('expired') ||
                                 error?.message?.includes('Expired') ||
                                 String(error).includes('ExpiredToken')

          if (this.debug) {
            console.log(colors.warning(`[DEBUG] Cached auth failed: ${error?.error || error?.message || error}`))
            console.log(colors.warning(`[DEBUG] Is expired token: ${isExpiredToken}`))
          }

          if (isExpiredToken) {
            if (this.debug) {
              console.log(colors.warning('[DEBUG] Cached token expired, attempting refresh...'))
            }
            try {
              ...
              if (refreshed.success) {
                ...
                await this.setupSession(cachedHandle, cached.did, refreshed.data.accessJwt, refreshed.data.refreshJwt, pdsUrl)
                return
              }
            } catch (refreshError: any) {
              if (this.debug) {
                console.log(colors.warning(`[DEBUG] Token refresh failed: ${refreshError?.message || refreshError}`))
              }
            }
            // Clear invalid cache entry
            delete this.authCache[cachedHandle]
            this.saveAuthCache()
            console.log(colors.warning('Session expired. Please log in again.'))
            // Fall through to fresh authentication
          } else {
            // For non-token-expiry errors, also fall through but log it
            if (this.debug) {
              console.log(colors.warning('[DEBUG] Non-expiry auth error, will re-authenticate'))
            }
            delete this.authCache[cachedHandle]
            this.saveAuthCache()
            console.log(colors.warning('Authentication issue. Please log in again.'))
            // Fall through to fresh authentication
          }
        }
      }
      ...
      await this.setupSession(resolvedHandle, response.data.did, response.data.accessJwt, response.data.refreshJwt, pdsUrl)
  ...
  private async setupSession(handle: string, did: string, accessJwt: string, refreshJwt: string, pdsUrl: string) {
    ...
    try {
      ...
      if (certs.endpointCerts && certs.endpointCerts.length > 0) {
        if (this.debug) {
          console.log(colors.info(`[DEBUG] Found ${certs.endpointCerts.length} endpoint cert(s) on PDS: ${certs.endpointCerts.map(c => c.serialNumber).join(', ')}`))
        }
        ...
        for (const cert of certs.endpointCerts) {
          if (cert.privateKey || cert.certificatePrivateKeyPEM) {
            ...
            hasPrivateKey = true
            if (this.debug) {
              console.log(colors.success('[DEBUG] ✓ Endpoint certificate with private key loaded'))
            }
            break
          }
        }
        ...
        if (!hasPrivateKey) {
          for (const cert of certs.endpointCerts) {
            const localCert = localCerts.find(
              c => c.type === 'endpoint' && c.serialNumber === cert.serialNumber
            )
            if (localCert?.hasPrivateKey && localCert.privateKeyPEM) {
              try {
                // Reconstruct proper ATSMSEndpointCertificate instance with private key
                ...
                if (localCert.isEncrypted) {
                  ...
                  hasPrivateKey = true
                  if (this.debug) {
                    console.log(colors.success('[DEBUG] ✓ Endpoint certificate found with encrypted private key in local storage'))
                    console.log(colors.info('[DEBUG]   (Password will be required when sending messages)'))
                  }
                } else {
                  ...
                  hasPrivateKey = true
                  if (this.debug) {
                    console.log(colors.success('[DEBUG] ✓ Endpoint certificate found with private key in local storage'))
                  }
                }
                break
              } catch (innerError: any) {
                console.log(colors.error(`Failed to create cert with key: ${innerError.message}`))
              }
            }
          }
        }
      ...
      console.log(colors.error(`⚠ Could not check certificates: ${error?.message || error}`))
      if (error?.stack) {
    ...
    if (endpointCert && endpointCert.certificatePrivateKeyPEM) {
      try {
        ...
        wsClient = await storageManager.createWebSocketClient(endpointCert, {
          onError: (error: Error) => {
            if (this.debug) {
              ...
              console.log(colors.warning(`⚠ WebSocket connection issue: ${error.message || 'Connection failed'}`))
            }
          },
          onDisconnect: (code: number, reason: string) => {
            if (this.debug) {
              console.log(colors.system(`[DEBUG] WebSocket closed: ${code} - ${reason}`))
            } else if (code !== 1000) {
        ...
        if (this.debug) {
          ...
          console.log(colors.warning(`⚠ Could not connect to WebSocket: ${error.message || 'Connection failed'}`))
        }
      }
    } else if (endpointCert) {
      console.log(colors.warning('⚠ WebSocket not connected - private key not available'))
    }
  ...
  private async displayNewMessage(messageId: string) {
    ...
    try {
      ...
      if (message) {
        ...
        const time = colors.timestamp(new Date(message.createdAt).toLocaleTimeString())
  ...
  private async showNotification(convoId: string, messageId: string) {
    ...
    try {
      const message = await (this.state.storageManager as any).storage.getMessage(messageId)
  ...
  private async resolveHandleFromDid(did: string): Promise<string> {
    ...
    if (this.state?.agent) {
      try {
        const response = await this.state.agent.api.app.bsky.actor.getProfile({ actor: did })
  ...
  private async populateHandleCache() {
    ...
    try {
      ...
      for (const conv of conversations) {
        const otherParticipant = conv.participantIds.find(id => id !== this.state!.did)
        if (otherParticipant) {
  ...
  private async handleCommand(command: string) {
    ...
    switch (cmd) {
      ...
      default:
        console.log(colors.error(`Unknown command: ${cmd}. Type '/?' for help`))
    }
  }
...
`))
  ...
  private async enterConversation(recipient: string) {
    ...
    try {
      ...
      if (!recipient.startsWith('did:')) {
        ...
        try {
            ...
            for (const pdsUrl of pdsUrls) {
              try {
                ...
                if (resolved?.data?.did) {
                  ...
                  console.log(colors.system(`Resolved ${recipient} to ${recipientDid}`))
                  break
                }
              } catch {
                // Try next PDS
            ...
            if (!recipientDid || recipientDid === recipient) {
              console.log(colors.error(`Could not resolve handle ${recipient} to DID`))
              console.log(colors.info(`Please ensure the handle is correct and the user has an AT Protocol account`))
              return
            }
          } catch {
      ...
      console.log(colors.system(`Checking AT-SMS setup for ${recipientHandle}...`))
      try {
        const recipientCerts = await this.state.atsmsClient.getUserCertificates(recipientDid)

        if (recipientCerts.error === 'NOT_ATPROTO_USER') {
          console.log(colors.error(`✗ ${recipientHandle} is not a valid AT Protocol user`))
        ...
        if (recipientCerts.error === 'FETCH_ERROR') {
          console.log(colors.warning(`⚠ Could not verify AT-SMS setup for ${recipientHandle}`))
          console.log(colors.info('  Network error - continuing anyway, but sending messages may fail'))
        } else {
          // Success - show certificate info
          console.log(colors.success(`✓ ${recipientHandle} has ${recipientCerts.endpointCerts.length} endpoint certificate(s)`))
          if (this.debug) {
            console.log(colors.info(`[DEBUG] Endpoint cert serials: ${recipientCerts.endpointCerts.map(c => c.serialNumber).join(', ')}`))
          }
        }

      } catch (error: any) {
        console.log(colors.warning(`⚠ Failed to validate AT-SMS setup for ${recipientHandle}: ${error.message}`))
        console.log(colors.info('  Continuing anyway, but sending messages may fail'))
      }

      // Find or create conversation using library method
      const conversation = await this.state.storageManager.getOrCreateConversation([
        this.state.did,
        recipientDid
      ])
      ...
      ;(this.state as any).conversationHandles = (this.state as any).conversationHandles || {}
      ;(this.state as any).conversationHandles[conversation.id] = recipientHandle
  ...
  private async sendMessage(text: string) {
    ...
    const hasMemoryKey = this.state.endpointCert.privateKey ||
    ...
    if (!hasMemoryKey) {
      ...
      if (localCert && localCert.privateKeyPEM) {
        ...
        if (localCert.isEncrypted) {
          const password = await this.promptPassword('Enter password to decrypt endpoint certificate: ')
          if (!password) {
            console.log(colors.error('Password required to decrypt private key'))
            return
          }
          try {
            privateKeyPEM = PasswordEncryption.decrypt(localCert.privateKeyPEM, password)
          } catch {
        ...
        try {
          ...
          endpointCertWithKey = await ATSMSEndpointCertificate.fromPEMWithKey(
            localCertData.certificatePEM,
            privateKeyPEM
          )
        } catch {
          console.log(colors.error('Failed to load endpoint certificate with private key'))
          return
        }
      } else {
        console.log(colors.error('\n⚠ Your certificate doesn\'t have a private key'))
        console.log(colors.info('You have a certificate but cannot sign messages without the private key.'))
        console.log(colors.info('\nTo fix this:'))
        console.log(colors.info('  1. Type /back to exit conversation'))
        console.log(colors.info('  2. Type "/genendpoint" to generate a new endpoint certificate with private key'))
    ...
    try {
      const conversation = await this.state.storageManager.getConversation(this.state.currentConversation)
      if (!conversation) {
      ...
      // Get recipient DID
      const recipientDid = conversation.participantIds.find(id => id !== this.state!.did)
      if (!recipientDid) {
      ...
      if (errorMessage.includes('HTTPS send-message endpoint not yet implemented')) {
        if (!this.state.wsClient || !this.state.wsClient.isConnected()) {
          ...
          console.log(colors.info('\nTry:'))
          console.log(colors.info('  1. Type "/quit" and restart the chat client'))
          console.log(colors.info('  2. Check your internet connection'))
          if (this.debug) {
            console.log(colors.system(`[DEBUG] WebSocket client exists: ${!!this.state.wsClient}`))
            console.log(colors.system(`[DEBUG] WebSocket connected: ${this.state.wsClient?.isConnected()}`))
          }
        }
      }
    }
    ...
    try {
      ...
      for (const conv of conversations) {
        ...
        if (!otherParticipant || otherParticipant === 'Unknown') {
          console.log(`  Unknown participant - Last: ${new Date(conv.lastMessageAt).toLocaleString()}`)
          continue
        }
        ...
        if (handle.endsWith('...')) {
          try {
            ...
            const response = await this.state!.agent.api.app.bsky.actor.getProfile({ actor: otherParticipant })
            if (response.data.handle) {
        ...
        const unread = conv.unreadCount > 0 ? colors.error(` (${conv.unreadCount} unread)`) : ''
  ...
  private async syncMessages() {
    ...
    try {
      ...
      if (!this.state.endpointCert?.serialNumber) {
        ...
        console.log(colors.info('You need a endpoint certificate to receive messages'))
        return
      }

      // Check if we have the private key
      let privateKeyPEM: string | undefined = this.state.endpointCert.certificatePrivateKeyPEM

      // If private key is encrypted, we need to decrypt it
      if (!privateKeyPEM && (this.state.endpointCert as any).encryptedPrivateKeyPEM) {
        const password = await this.promptPassword('Enter password to decrypt private key: ')
        try {
          privateKeyPEM = PasswordEncryption.decrypt((this.state.endpointCert as any).encryptedPrivateKeyPEM, password)
        } catch {
      ...
      if (!privateKeyPEM) {
        console.log(colors.error('Endpoint certificate private key not available'))
        console.log(colors.info('You need the private key to authenticate with the server'))
        console.log(colors.info('\nTo fix this:'))
        console.log(colors.info('  1. If you have the key on another device, import it'))
        console.log(colors.info('  2. Or generate a new endpoint certificate: type "/genendpoint"'))
        return
      }

      // Temporarily set the decrypted private key on the certificate for the sync
      const originalPrivateKey = this.state.endpointCert.certificatePrivateKeyPEM;
      ...
      console.error(colors.error('Sync failed:'), error?.message || String(error))
    }
  }
...
`))
  ...
  private async generateEndpointCertificate() {
    ...
    try {
      ...
      const storage = this.state.storageManager.getStorage() as SQLiteAdapter
      if (password) {
        const encryptedKey = PasswordEncryption.encrypt(endpointCert.certificatePrivateKeyPEM!, password)
        await storage.saveCertificate(
          this.state.did,
          'endpoint',
      ...
      try {
        await this.state.storageManager.saveDid(this.state.did, this.state.handle, endpointCert)
        ...
        if (this.state.storageManager.isTransportActive(this.state.did)) {
          ...
          if (this.debug) {
            console.log(colors.info('[DEBUG] Stopped existing transport to pick up new certificate'))
          }
        }
        ...
        if (this.debug) {
          console.log(colors.success('[DEBUG] ✓ DID saved and transport started'))
        }
      } catch (transportError: any) {
        console.log(colors.warning(`⚠ Transport setup failed: ${transportError.message || 'Unknown error'}`))
    ...
    try {
      ...
      if (certs.endpointCerts && certs.endpointCerts.length > 0) {
        ...
        for (const cert of certs.endpointCerts) {
          ...
          const localClient = localCerts.find(c => c.type === 'endpoint' && c.serialNumber === cert.serialNumber)
          const hasLocalKey = localClient?.hasPrivateKey || false

          console.log(`  ${cert.serialNumber}`)
          console.log(`    Valid: ${cert.notBefore.toLocaleDateString()} - ${cert.notAfter.toLocaleDateString()}`)

          if (hasMemoryKey) {
            console.log(`    Has private key: ${colors.success('✓ Yes (can send messages)')}`)
          } else if (hasLocalKey) {
            console.log(`    Has private key: ${colors.success('✓ Yes (local storage, can send messages)')}`)
          } else {
            console.log(`    Has private key: ${colors.warning('✗ No (cannot send messages)')}`)
          }

          if ((hasMemoryKey || hasLocalKey) && !this.state.endpointCert) {
            // Use this certificate if we don't have one
            this.state.endpointCert = cert
            console.log(colors.info('    ↑ Using this certificate for sending'))
          }
        }
        console.log('')
      } else {
      ...
      console.log(`  Can send messages: ${canSend ? colors.success('✓ Yes') : colors.error('✗ No')}`)
      console.log(`  Can receive messages: ${canReceive ? colors.success('✓ Yes') : colors.error('✗ No')}`)
      ...
      console.log(colors.error('Failed to list certificates: ' + (error.message || 'Unknown error')))
    }
  }
  ...
  private async sendTestWebRTCOffer(mediaType: 'audio' | 'video' = 'audio') {
    ...
    try {
      ...
      const webrtcContent: ATSMSWebRTCContent = {
        type: 'offer',
        callId: Date.now().toString(36) + Math.random().toString(36).substr(2, 9),
        sdp: 'v=0\r\no=- 123456789 2 IN IP4 127.0.0.1\r\ns=Test WebRTC Offer\r\nt=0 0\r\na=group:BUNDLE 0\r\na=msid-semantic: WMS\r\nm=' +
             mediaType + ' 9 UDP/TLS/RTP/SAVPF 111\r\nc=IN IP4 0.0.0.0\r\na=rtcp:9 IN IP4 0.0.0.0\r\na=ice-ufrag:test\r\na=ice-pwd:testpassword\r\n',
      ...
      console.log(colors.success(`✓ Sent ${mediaType} call offer (test only - no real WebRTC connection)`))
    } catch (error: any) {
      console.error(colors.error(`Failed to send call offer: ${error.message}`))
    }
  }
