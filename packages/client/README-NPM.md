...
- 🔒 **JWT Authentication**: Certificate-signed JWTs for ATSMS API authentication
...
// 2. Create ATSMS client (for certificate management)
...
2. **Client Certificates** (RSA 2048)
   ...
   - Used for message encryption AND ATSMS API authentication
...
ATSMS uses **two distinct JWT types**:
...
### 2. ATSMS API JWTs (Internal to Library)
- **Purpose**: Authenticate with ATSMS inbox service
...
// ATSMS library generates these internally
...
All ATSMS types use the `ATSMS` prefix:
...
- **ATSMS Service**: Access to ATSMS inbox service endpoint
